...
### OpenCode — merged-trigger skill variants + condux plugin

OpenCode surfaces only `description` when deciding which skill to load, so the
plain `skills/` tree loses the `when_to_use` trigger conditions many skills
carry. Install the OpenCode-facing variants instead — same skills, with the
trigger conditions folded into each description:
...
packages/condux-opencode/ # OpenCode plugin (npm) — condux agents + plan-review listener
.claude-plugin/
...
- **OpenCode** installs from **`dist/opencode/skills/`** (see the OpenCode
  install section) — generated by `scripts/build-opencode.mjs`, which
  `scripts/sync.sh` runs automatically.
