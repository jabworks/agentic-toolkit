...
**Every implementation request starts at the `workflow` skill — `skill(name="workflow")`.** Feature, bug fix,
...
you have skipped the router — go back to the `workflow` skill — `skill(name="workflow")`. Debugging is the
...
**On OpenCode, a `/name` mention of any condux skill means the `skill` tool:** `/workflow` → `skill(name="workflow")`, `/finalize` → `skill(name="finalize")`, and so on. None of them is a slash command here.
</EXTREMELY_IMPORTANT>
