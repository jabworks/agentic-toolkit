...
For LARGE tasks (cross-cutting, unclear scope, multiple subsystems), `/discovery` should have already run before you are invoked — it produces the design sign-off that you turn into an executable plan. If you receive a vague or under-defined request for a large task, ask whether discovery has run first rather than attempting to fill in the design yourself.
