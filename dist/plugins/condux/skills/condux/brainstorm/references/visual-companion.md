...
Locate the script from the installed `brainstorm` skill and launch it in the
background, pointing at the mockup file:

```bash
CHOICE=$(find ~/.claude ~/.agents ~/.codex -name "choice-server.js" -path "*/brainstorm/*" 2>/dev/null | head -1)
node "$CHOICE" /tmp/brainstorm-checkout-flow-202601.html          # single-select
node "$CHOICE" /tmp/brainstorm-checkout-flow-202601.html --multi  # multi-select
```
...
After the user reviews the visual, return to the brainstorm sign-off flow. If
...
chosen, then continue with `/write-plan` or the next step.
