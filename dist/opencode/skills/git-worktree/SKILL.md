---
name: git-worktree
description: Use when working with git worktrees — creating an isolated workspace for a task or agent, listing or switching trees, moving work between them, pruning stale ones, recovering a broken one. Native-first — prefer the host's own worktree tooling, fall back to `git worktree` only when there is none. Not for undo/discard/stash/merge/push (git-operations). Not for fanning agents across trees (subagent-deployment).
---
