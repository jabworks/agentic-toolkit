...
packages/condux-opencode/     # OpenCode npm plugin — injects condux agents via
                              # the config hook; agents/ is generated from
...
- `opencode-dist.test.mjs` — `dist/opencode/skills/` and
  `packages/condux-opencode/agents/` match `scripts/build-opencode.mjs` output,
  merged descriptions stay within OpenCode's 1024-char cap, the plugin loads
  and never clobbers user-defined agents
...
package (source in `packages/condux-opencode/`) for agents. `dist/` is a build artifact — edit `skills/`, then
