...
| `close <id> [--note "<verification>"]` | Stamp `— ✅ DONE <today>` + verification note, move the entry from DOCKET.md to `archive/<year>.md` (create file with header if absent), update `docket.json`; prints suggested commit subject `docs(docket): close #<id>`. `<id>` is a number, or the exact id slot of a qualified item (`close "47 (remainder)"`). A number matching more than one open item is an error listing the candidates — never a first-match guess, because closing is a move with no undo |
| `check` | Id-space integrity: duplicate **allocations**, `next_id` drift, malformed headings, and references (`#47 (remainder)`) whose parent is not allocated (`orphan-reference`); exit non-zero on any finding. A qualified id references its parent rather than allocating, so a parent and its slices are never reported as duplicates |
