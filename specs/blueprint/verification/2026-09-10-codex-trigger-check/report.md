...
| The delivered diagram renders with no edge through a box and no label on node text | `trial-1.html`, `trial-2.html`, `trial-3.html` opened with `google-chrome --headless=new --screenshot` at a window large enough for the whole canvas (2300×1200 for trial 1's 2180-wide viewBox; 1900×2700 for trial 3's three stacked canvases); light scheme; screenshots observed in-session, not committed | ✓ 3/3 — every edge in a gutter, every label haloed; trial 3 split the system into three sectioned canvases (topology, event flows, storage ownership) with dashed boundary boxes |
...
- **Trial 3 packed three canvases into one file** (topology, event flows, storage ownership — three `<svg>` blocks, each with its own `viewBox` from 0,0) where the kit says one file per diagram. The checker walks each `<svg>…</svg>` block on its own, so overlapping coordinates across blocks cannot collide; splitting the file into one canvas each and re-running gives `clean` three times. Whether the kit should allow or forbid the sectioned shape is a #78 question — it reads well.
